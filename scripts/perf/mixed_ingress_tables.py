...
def main():
    ...
    table(['Protocol','N','Accepted/s','Range','ACK/sent %','ACK/offered %','P95 ms','P99 ms'],rows)
    add('## TLS mixed throughput and resources','','Each key mixed run lasts 300 measurement seconds; warmup is excluded. CPU is process CPU time expressed in cores. RSS is the median run peak.','')
    rows=[]
    ...
    add('## Fairness and acknowledged latency','','Retained capacity = mixed accepted/s divided by baseline solo accepted/s. Reduced allocated share alone is not starvation. Wire failure % excludes generator schedule/window drops; offered coverage in the preceding table includes them.','')
    rows=[]
    for scenario in ['balanced','mqtt-heavy','http-heavy','tcp-heavy','udp-heavy']:
        r=existing('baseline',scenario)
        if not r: continue
    ...
    table(['Scenario','Protocol','Solo/s','Mixed/s','Retained','P95 ms','P99 ms','Wire failure %'],rows)
    ...
    table(['Scenario','Before N','After N','Before/s','After/s','Delta'],rows)
    add('## Connection interference','','Values are accepted/offered coverage except `new MQTT auth`, which is application-authenticated/connection-attempt percentage during measurement. TCP/TLS connection success is also preserved separately in JSON. Blank means no such probe, not zero errors.','')
    rows=[]
    for variant,scenario in [('baseline','idle-256'),('candidate','idle-256-v3'),('final','idle-final-256'),
    ...
    add('## Offered-load staircases','','Exploratory steps are one 30-second run each; repeated saturation trials above validate the connection cliff. Percentages refer to the calibrated offered operating points (HTTP 8,000/s; UDP 80,000/s), or 256 idle connections for occupancy. Actual sent rates and window misses remain in JSON.','')
