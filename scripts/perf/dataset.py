...
DO $quota$
BEGIN
  IF to_regclass('public.ingress_quota_global') IS NOT NULL THEN
    UPDATE ingress_quota_global SET messages=(SELECT count(*) FROM ingress_messages),bytes=(SELECT coalesce(sum(charge),0) FROM ingress_messages) WHERE singleton;
    INSERT INTO ingress_quota_tenants(tenant_id,messages,bytes)
      ...
      ON CONFLICT (tenant_id) DO UPDATE SET messages=excluded.messages,bytes=excluded.bytes;
    INSERT INTO ingress_quota_devices(tenant_id,product_id,device_id,messages,bytes)
      SELECT tenant_id,product_id,device_id,count(*),sum(charge) FROM ingress_messages GROUP BY tenant_id,product_id,device_id
      ON CONFLICT (tenant_id,product_id,device_id) DO UPDATE SET messages=excluded.messages,bytes=excluded.bytes;
  END IF;
END
$quota$;
...
DO $quota$
BEGIN
  IF to_regclass('public.command_quota_global') IS NOT NULL THEN
    UPDATE command_quota_global SET commands=(SELECT count(*) FROM commands) WHERE singleton;
    INSERT INTO command_quota_tenants(tenant_id,commands)
      SELECT tenant_id,count(*) FROM commands GROUP BY tenant_id
      ON CONFLICT (tenant_id) DO UPDATE SET commands=excluded.commands;
    INSERT INTO command_quota_devices(tenant_id,product_id,device_id,commands)
      SELECT tenant_id,product_id,device_id,count(*) FROM commands GROUP BY tenant_id,product_id,device_id
      ON CONFLICT (tenant_id,product_id,device_id) DO UPDATE SET commands=excluded.commands;
  END IF;
END
$quota$;
"""
...
def queries(now):
    ...
    return {
      ...
      'quota_counters':"SELECT g.messages,g.bytes,t.messages,t.bytes,d.messages,d.bytes FROM ingress_quota_global g CROSS JOIN ingress_quota_tenants t JOIN ingress_quota_devices d USING(tenant_id) WHERE g.singleton AND t.tenant_id='seed1' AND d.product_id='p' AND d.device_id='d1'",
...
def run_scale(rows, optimized=False):
    database=f'{"opt" if optimized else "cap"}_dataset_{rows}';subprocess.run([PG+'/createdb',database],env=ENV,check=True,capture_output=True)
    pg(database,(ROOT/'migrations/0001_foundation.sql').read_text())
    if optimized:
        pg(database,(ROOT/'migrations/0002_ingress_quota_accounting.sql').read_text())
    ...
    selected_queries=queries(fixture_time+10000)
    if not optimized:
        selected_queries.pop('quota_counters')
    ...
    folder=ROOT/'docs/performance'/('optimization' if optimized else '')
    folder.mkdir(parents=True,exist_ok=True)
    path=folder/f'dataset_{rows}.json';path.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(dict(rows=rows,bytes=result['database_bytes'],queries={k:v['median_ms'] for k,v in result['queries'].items()})),flush=True)

if __name__=='__main__':
    optimized='--optimized' in sys.argv
    sizes=[int(value) for value in sys.argv[1:] if value.isdigit()] or [10000,100000,1000000]
    for rows in sizes:run_scale(rows,optimized)
