...
One request may present only one credential. A failed provider is never retried through another provider. The static token produces `bootstrap-admin`, `admin.*`, and Global resource access. Set `management_auth.legacy_static_token_enabled` to `false` after replacing it. Without a configured provider, management stays closed.
...
`secret_env` must resolve at startup to 64 hexadecimal characters. `expires_at` is Unix milliseconds. `global: true` is explicit; an empty resource list grants no resource access. Key IDs, subjects, scopes, and resource sets are validated and bounded at startup. Treat API Keys as high entropy random values.
