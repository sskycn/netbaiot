...
| MQTT-SIZE | P1 | yes; final bound corrected in follow-up | the prior 256 MiB check counted logical bytes but JSON `Vec<u8>` can expand to four characters per byte and strings to six | `mqtt_recovery_upper_bound()` proves a six-times logical-state ceiling plus bounded record wrappers; default/config are 1.25 GiB | high-byte (`00/7f/80/ff`) commit/recover and >1 MiB legal-state tests pass | one bounded image is memory-loaded during planned recovery; compact binary remains a future optimization |
