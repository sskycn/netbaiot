...
| MQTT-SIZE | P1 | yes; superseded by correctness freeze | v1 cloned and JSON-expanded the complete state | compact streaming NBMQ v2 with raw payloads and bounded records; default/config 192.25 MiB | 10/50/100 MiB benchmark, v1/v2 round trips, actual decoder fuzz | v1 remains read-only compatible and bounded |
...
| MQTT-ACK-MATRIX | P1 | yes | boolean completion treated PUBACK against AwaitPubrec as valid and removed the QoS2 transaction | explicit `OutboundAck` × `OutboundState` matching; wrong ACK is mutation-free | `QOS2-OUT-WRONG-PUBACK-001`, `QOS2-OUT-WRONG-PUBCOMP-001` pass | protocol-error connection policy remains intentional |
| MQTT-QOS2-TAKEOVER | P1 | yes | EventAccepted completion was fenced by the old connection generation | bounded operation token owns `Delivering`; accepted completion and routing are session-transaction operations across takeover | `QOS2-IN-SESSION-TAKEOVER-001` passes deterministically | business delivery remains at-least-once across abnormal process loss |
| AUTH-REGISTER-RACE | P1 | yes | invalidation could occur after auth returned but before `Sessions::register` | auth candidates carry a cache epoch; invalidation and registration share a short mutex gate | all six invalidation scopes pass `AUTH-REGISTER-REVOKE-RACE-001` | global epoch conservatively rejects unrelated stale candidates |
| MQTT-ATTACH | P1 | yes | post-attach CONNACK/write errors could leave active MQTT ownership | generation-fenced `Attachment` Drop cleanup; persistent storage and clean-session lifetime stay distinct | real closed-duplex CONNACK failure covers CleanSession 0/1 and subsequent Session Present | a poisoned process mutex is unrecoverable process failure |
| MQTT-TENANT-WAKE | P1 | yes | an ACK polled only its own offline queue, so another same-tenant session could remain stuck | per-tenant deduplicated bounded ready queue; ACK release directly schedules another active session without a global scan/task | QoS1 and QoS2 cross-session wake IDs pass | FIFO front preserves per-session ordering and can intentionally wait for its QoS class |
| MQTT-WILL-LEASE | P1 | yes | accepted Will publication could fail later because retained capacity was consumed | CONNECT reserves retained capacity; armed `WillGuard` publishes on abnormal Drop and suppresses only on DISCONNECT | `WILL-RESOURCE-FAILURE-001` and raw Will shutdown/takeover tests pass | optional IoT binding remains best effort after MQTT publication |
| EVENT-SPIN | P2 | yes | ready queued work plus full sink concurrency selected a zero-delay timer repeatedly | full concurrency waits only for stop or an inflight completion | `EVENTBUS-CONCURRENCY-NOSPIN-001` passes | ready selection remains bounded O(queue) below concurrency |
| RELEASE-GATE | P2 | yes | unknown `--only` and missing external dependencies could yield a misleading success | exact-ID/duplicate checks, required dependency failures, stable Rust evidence IDs, and 125-entry machine traceability | full gate 65/65 PASS; unknown ID exits nonzero | remote Actions still requires a user push |
...
Persistent MQTT overload is now producer-visible and atomic instead of subscriber
shedding for QoS1/QoS2. Persistent authorization provenance and session incarnation
are restored and revalidated. NBMQ v2 writes compact bounded records and v1 remains
