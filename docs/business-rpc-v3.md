...
Stream 0 is limited to PING, PONG, connection WINDOW_UPDATE and GOAWAY. Client IDs are odd, gateway IDs even, strictly increasing and never reused within one connection. Once the last legal local ID is consumed, the endpoint sends GOAWAY NO_ERROR and reconnects instead of wrapping. The stream identity is `(connection_epoch, stream_id)`; application `request_id`, stable `event_id`, `delivery_id` and `subscription_id` retain separate meanings. GOAWAY's `last_stream_id` reports the highest peer stream considered. A reconnect receives a new epoch.

Provider and EventSubscription are long-lived parent streams. A provider parent admits gateway-initiated child RPCs for `device.authenticate` and `device.resolve_verifier`, and client-initiated child RPCs for `auth.sync` and `auth.invalidate`. The provider becomes Serving only after reset sync completes and the client confirms receipt through the PING/PONG barrier. An EventSubscription parent admits gateway-initiated EventDelivery children. One delivery per subscription remains in flight; the application calls `BusinessRpcV3Delivery::ack()` only after its commit, or `nack()` on failure. A socket write, WINDOW_UPDATE or receipt by the SDK is not the business ACK. Retries may change `delivery_id` while `event_id` stays stable.

## Online commands

`device.command.send` is an independent client-initiated RPC with no parent stream. OPEN carries the method, a fresh `request_id`, a deadline and the declared body length; DATA carries JSON `DeviceCommandSendRequest { command }` and END_STREAM. The gateway returns `DeviceCommandSendResponse { dispatch }` in RESPONSE/DATA. The principal must list `device.command.send` in `call_methods` and allow the command's tenant. Method authorization precedes DTO decoding; tenant authorization precedes session lookup. No new V3 frame or mux rule is involved.

Use `BusinessRpcV3Client::send_command(&command)`. A `Queued` result means the current local MQTT/TCP device session accepted dispatch; it does not mean the device received or executed it. The device's later `CommandAck` arrives as a normal EventDelivery and is matched by `command_id`. A connection loss or timeout after submission is `OutcomeUnknown`; retry the identical command with the same `command_id` after reconnect. Each RPC attempt has a new `request_id` and stream ID. Conflicting contents for one `(tenant_id, command_id)` return `Conflict`; offline or command-unready devices return `Unavailable`. RESET_STREAM after dispatch does not cancel the device command. New commands are rejected during drain, while a retained accepted receipt may be read again.

HTTP `/api/v1/devices/commands`, V2, and V3 share one `CommandService` and one process-local dedup registry. `command_dedup_max_entries` defaults to 4096 and `command_dedup_ttl_ms` to 300000. Within that window, identical retries return the original receipt without a second dispatch. Capacity exhaustion returns `Overloaded`; failed dispatch does not reserve the ID. The registry is not persisted: a crash or restart loses dedup history, and no crash-surviving exactly-once guarantee exists. Keep business-side command history and offline retry policy outside the gateway.
...
The gateway can optionally bound sender-local DATA submitted before peer progress without reducing the negotiated receive windows. Set `business_rpc.v3_send_ahead` to `{"stream_bytes": 16384, "connection_bytes": 131072}` for an experiment. Both values must be at least the configured frame payload, the connection value must be at least the stream value, and the connection value may not exceed 16 MiB. Invalid configurations fail startup. The default is `null`, which preserves the original window-only scheduling behavior. Stream `WINDOW_UPDATE` releases the corresponding stream counter; connection `WINDOW_UPDATE` releases the connection counter. Local `AsyncWrite` completion does not release either counter. The current receiver sends an update after every non-final DATA frame, so an 8 KiB sender limit with an 8 KiB frame does not wait for a larger batching threshold. On RESET, queued body bytes and the stream counter are removed; submitted connection bytes remain charged until peer connection credit arrives or the connection closes. This is a local policy and adds no Hello setting or wire field. A sender can only reorder frames that it has not yet submitted to the ordered TLS/TCP stream.
