...
EventBus 当前 writer 为 v2，并可读 legacy v1。MQTT 当前 writer 是 streaming NBMQ v5，读取 v1/v2/v3/v4/v5；snapshot 包含权威 record-count/byte-count/digest trailer。目录默认应为 0700、文件 0600；监控容量、权限、inode 和本地磁盘错误，不要把它当 hot-path queue 或一般 event store。
